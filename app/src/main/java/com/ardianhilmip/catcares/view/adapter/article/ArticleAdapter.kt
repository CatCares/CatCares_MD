...
class ArticleAdapter :
    PagingDataAdapter<DataItem, ArticleAdapter.MyViewHolder>(DIFF_CALLBACK) {

    class MyViewHolder(private val binding: ItemArtikelVerticalBinding) :
        RecyclerView.ViewHolder(binding.root) {
        fun bind(article: DataItem) {
            binding.apply {
                tvItemTitle.text = article.judul
                tvItemDate.text = article.createdAt.withDateFormat()
                Glide.with(itemView)
                    .load(article.foto)
                    .into(ivItemPhoto)
